...
[Etherscan CLI](https://github.com/etherscan/etherscan-cli) v1.0.0 or newer and log in once:
...
API key stays in `ETHERSCAN_API_KEY` or the CLI's local config file, and the skill calls commands such
as:
...
The skill pages these itself rather than using `--all`, so it can stop the moment a branch reaches a CEX, mixer, or bridge and preserve one fetch-log entry per API response. Production v1's `--all` combines pages into one result, defaults to a 20-page cap, and only reports cap truncation on stderr.
