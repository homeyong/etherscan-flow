...
| Chain hint | "on Base", "a Polygon token" | feed into the *Chain resolution* procedure in `SKILL.md`: validate the chain is V2-supported (chain table, else one `chainlist` call accepting status `1` available or `2` degraded — a status `2` chain stays usable with a `chain_degraded` gap); if no chain is mentioned anywhere, default to mainnet (chainid 1); if the only named chain is not V2-supported (status `0` or absent), that is a blocker — never trace it on mainnet instead |
