...
1. **Validate before you call.** Reject any input that does not match: address `^0x[a-fA-F0-9]{40}$`, tx hash `^0x[a-fA-F0-9]{64}$`, apikey `^[A-Za-z0-9]{1,64}$`, chainid present in the chain table or confirmed with status `1` (available) or `2` (degraded) by this run's `chainlist` response (see *Chain resolution*). Never build a URL from an unvalidated value.
...
| Named chain not V2-supported | The only chain in the input is not in the chain table and is absent from this run's `chainlist` response or has status `0` (offline) | If a supported chain is also in scope, trace it and add the `chain_unsupported` gap; a status `2` chain remains usable with a `chain_degraded` gap; never silently substitute mainnet for a named chain |
